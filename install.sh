...
#   curl -fsSL https://raw.githubusercontent.com/ZurabDev/zagents-releases/main/install.sh | bash -s -- --with-server
#
# After installation, run `zagents setup` to configure your environment.
#
set -euo pipefail
...
REPO_URL="https://github.com/ZurabDev/multica.git"
# mz: public-releases — release assets (CLI archives, checksums, `releases/latest`)
# live in the public ZurabDev/zagents-releases mirror; ZurabDev/multica is
# private and 404s for anonymous consumers.
REPO_WEB_URL="https://github.com/ZurabDev/zagents-releases"  # without .git, for GitHub web APIs
INSTALL_DIR="${ZAGENTS_INSTALL_DIR:-$HOME/.zagents/server}"
BREW_TAP="ZurabDev/zagents"            # mz: fork Homebrew tap
BREW_PACKAGE="ZurabDev/zagents/zagents" # mz: formula name (installs the `zagents` binary)
...
install_cli_brew() {
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      ...
      warn "Failed to install zagents via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
...
install_cli_binary() {
  ...
  # mz: zagents-rebrand — releases ship `zagents-cli-*` archives; keep the old
  # `multica-cli-*` name as a fallback for pre-rename releases.
  local url="${REPO_WEB_URL}/releases/download/${latest}/zagents-cli-${version}-${OS}-${ARCH}.tar.gz"
  local legacy_url="${REPO_WEB_URL}/releases/download/${latest}/multica-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/zagents.tar.gz"; then
    info "zagents-cli archive not found, trying legacy name $legacy_url ..."
    if ! curl -fsSL "$legacy_url" -o "$tmp_dir/zagents.tar.gz"; then
      rm -rf "$tmp_dir"
      fail "Failed to download CLI binary."
    fi
  fi

  tar -xzf "$tmp_dir/zagents.tar.gz" -C "$tmp_dir" zagents
  ...
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/zagents" "$bin_dir/zagents"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/zagents" "$bin_dir/zagents"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/zagents" "$bin_dir/zagents"
    chmod +x "$bin_dir/zagents"
  ...
  rm -rf "$tmp_dir"
  ok "Multica CLI installed to $bin_dir/zagents"
}
...
install_cli() {
  if command_exists zagents; then
    local current_ver
    # `zagents version` outputs "zagents v0.3.23-mz.1 (commit: …, built: …)" — extract just the version
    current_ver=$(zagents version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ...
    local new_ver
    new_ver=$(zagents version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
  ...
  # Verify
  if ! command_exists zagents; then
    fail "CLI installed but 'zagents' not found on PATH. You may need to restart your shell."
  fi
}
...
run_default() {
  ...
  printf "\n"
  printf "     ${CYAN}zagents setup${RESET}                # Connect to ZAgents Cloud (zagents.cnt.me)\n"  # mz: zagents-rebrand
  printf "     ${CYAN}zagents setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  print_remote_server_token_hint
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/ZurabDev/zagents-releases/main/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  ...
  printf "\n"
  printf "     ${CYAN}zagents setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/ZurabDev/zagents-releases/main/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  ...
  if command_exists zagents; then
    zagents daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo "  ZAGENTS_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.zagents/server)"
        ...
        echo ""
        echo "After installation, run 'zagents setup' to configure your environment."
        exit 0
        ;;
